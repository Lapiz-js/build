...
if($L.typeCheck.func(fn)&&$L.typeCheck.str(name)){$L.assert(name!=="","SetterMethod name cannot be empty string");}else if($L.typeCheck.func(name)&&name.name!==""){bind=fn;fn=name;name=fn.name;}else{Lapiz.Err.toss("SetterMethod requires either name and func or named function");}
if(bind!==undefined){fn=fn.bind(bind);}
...
Object.defineProperty(obj,property,desc);if(values&&Object.hasOwnProperty.call(values,property)){desc.set(values[property]);}}}});$L.set.meth($L.set,function binder(proto,name,fn){if(fn===undefined){fn=name;name=fn.name;}
...
return self;});$L.set.meth($L,function remove(arr,el,start){var i=arr.indexOf(el,start);if(i>-1){arr.splice(i,1);}});$L.set.meth($L,function each(obj,fn){var i;if($L.typeCheck.arr(obj)){var l=obj.length;for(i=0;i<l;i+=1){if(fn(obj[i],i)){return i;}}
...
cls.on.create(function(obj){obj.on.change(_upsert);obj.on.remove(function(obj){if($L.typeCheck.nested(obj,"on","change","deregister","func")){obj.on.change.deregister(_upsert);}
...
var _newClassEvent=$L.Event();$L.Event.linkProperty($L.on,"Cls",_newClassEvent);$L.set.meth($L,function Cls(classDef){var pub=Object.create(_clsDefProto);var priv=$L.Map();priv.props=$L.Map();priv.methods=$L.Map();_classBuilderWM.set(pub,priv);classDef.apply(pub,[pub]);var _clsProto=$L.Map();$L.each(priv.props,function(val,key){lateBindProp(_clsProto,key,val);});$L.each(priv.methods,function(val,key){lateBindMeth(_clsProto,key,val);});var _constr=priv.constructor;var constructor=function(){var self=Lapiz.Obj(_clsProto);if(_constr){self=_constr.apply(self,arguments)||self.pub;}else{self=self.pub;}

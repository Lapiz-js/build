...
val=undefined;var self=function(key,val){if(val===undefined){try{return _dict[key];}catch(err){Lapiz.Err.toss(err);}}
...
return newVal;});});Lapiz.Module("Events",["Collections"],function($L){$L.set($L,"Event",function(){var _listeners=[];var event=Lapiz.Map();$L.set.setterMethod(event,function register(fn){$L.typeCheck.func(fn,"Event registration requires a function");_listeners.push(fn);return fn;});$L.set.setterMethod(event.register,function deregister(fn){$L.remove(_listeners,fn);return fn;});$L.set.getter(event.register,function length(){return _listeners.length;});$L.set.meth(event,function fire(){if(!event.fire.enabled){return event;}
var i;var listeners=_listeners.slice(0);var l=listeners.length;for(i=0;i<l;i+=1){listeners[i].apply(this,arguments);}
return event;});$L.set.setterGetter(event.fire,"enabled",true,function(enable){return!!enable;});$L.set(event,"_cls",$L.Event);return event;});$L.set($L,"SingleEvent",function(){var _event=$L.Event();var _hasFired=false;var _args;var facade=$L.Map();$L.set.meth(facade,function register(fn){if(_hasFired){fn.apply(this,_args);}else{_event.register(fn);}});$L.set.meth(facade.register,function deregister(fn){if(_hasFired){return;}
...
$L.set.meth(self,function kill(){accessor.on.insert.deregister(inFn);accessor.on.remove.deregister(remFn);accessor.on.change.deregister(changeFn);if($L.typeCheck.nested(filterFn,"on","change","deregister","func")){filterFn.on.change.deregister(self.ForceRescan);}});$L.set(self,function Accessor(key){return self(key);});$L.set.copyProps(self.Accessor,self,"Accessor","&length","has","each","on","Sort","Filter","GroupBy","&keys");$L.set(self.Accessor,"_cls",$L.Accessor)
accessor.each(function(val,key){if(filterFn(key,accessor)){_index.push(key);}});Object.freeze(self);Object.freeze(self.Accessor);return self;});});Lapiz.Module("Group",function($L){$L.set($L,"Group",function(accessor,settings){var _index=[];var self=function(key){if(_index.indexOf(key.toString())>-1){return accessor(key);}};$L.set(self,"_cls",$L.Group);$L.set.meth(self,function Sort(funcOrField){return $L.Sort(self,funcOrField);});$L.set.meth(self,function Filter(filterOrField,val){return $L.Filter(self,filterOrField,val);});$L.set.meth(self,function GroupBy(funcOrField){return $L.GroupBy(self,funcOrField);});$L.set.meth(self,function has(key){return _index.indexOf(key.toString())>-1;});$L.set.getter(self,function keys(){return _index.slice(0);});$L.set.getter(self,function length(){return _index.length;});$L.set.meth(self,function each(fn){var i;var l=_index.length;for(i=0;i<l;i+=1){key=_index[i];if(fn(accessor(key),key)){return key;}}});$L.set(self,"on",$L.Map());var _insertEvent=$L.Event.linkProperty(self.on,"insert");var _changeEvent=$L.Event.linkProperty(self.on,"change");var _removeEvent=$L.Event.linkProperty(self.on,"remove");Object.freeze(self.on);function remFn(key,accessor,oldVal){key=key.toString();var i=_index.indexOf(key);if(i>-1){_index.splice(i,1);_removeEvent.fire(key,self,oldVal);}}
function changeFn(key,accessor,oldVal){key=key.toString();if(_index.indexOf(key)>-1){_changeEvent.fire(key,self,oldVal);}}
if(settings===undefined||settings.DoNotListen!==true){accessor.on.remove(remFn);accessor.on.change(changeFn);$L.set.meth(self,function kill(){accessor.on.remove.deregister(remFn);accessor.on.change.deregister(changeFn);});}
$L.set.meth(self,function Add(key){key=key.toString();if(accessor.has(key)&&_index.indexOf(key)===-1){_index.push(key);_insertEvent.fire(key,self);}});$L.set.meth(self,function Remove(key,oldVal){key=key.toString();var i=_index.indexOf(key);if(oldVal===undefined){oldVal=accessor(key);}
if(i>-1){_index.splice(i,1);_removeEvent.fire(key,self,oldVal);}});$L.set.meth(self,function Change(key,oldVal){changeFn(key,accessor,oldVal)})
$L.set(self,function Accessor(){return self(key);});$L.set.copyProps(self.Accessor,self,"Accessor","&length","has","each","on","Sort","Filter","&keys");self.Accessor._cls=$L.Accessor;Object.freeze(self.Accessor);Object.freeze(self);return self;});});Lapiz.Module("GroupBy",["Collections"],function($L){var _groupSettings=$L.Map();_groupSettings.DoNotListen=true;$L.set($L,"GroupBy",function(accessor,fieldOrFunc){var _groups=$L.Map();var _keymap=$L.Map();var _groupByFn=fieldOrFunc;if($L.typeCheck.str(fieldOrFunc)){_groupByFn=function(key,accessor){return accessor(key)[fieldOrFunc];};}
$L.typeCheck.func(_groupByFn,"GroupBy must be invoked with function or attriubte name");var self=function(key){key=key.toString();var group=_groups[key];if(group===undefined){group=$L.Group(accessor,_groupSettings);_groups[key]=group;}
return group.Accessor;};$L.set(self,"_cls",$L.GroupBy);$L.set.meth(self,function has(key){return _groups[key]!==undefined;});$L.set.getter(self,function keys(){return Object.keys(_groups);});$L.set.getter(self,function length(){return Object.keys(_groups).length;});function inFn(key,accessor){key=key.toString();var groupKey=_groupByFn(key,accessor);if(groupKey===undefined){return;}
var group=_groups[groupKey];if(group===undefined){group=$L.Group(accessor,_groupSettings);_groups[groupKey]=group;}
group.Add(key);_keymap[key]=groupKey;}
function remFn(key,accessor,oldVal){key=key.toString();var groupKey=_keymap[key];if(groupKey===undefined){return;}
var group=_groups[groupKey];group.Remove(key,oldVal);delete _keymap[key];}
function changeFn(key,accessor,oldVal){key=key.toString();var group;var oldGroupKey=_keymap[key];var newGroupKey=_groupByFn(key,accessor);if(oldGroupKey!==undefined){group=_groups[oldGroupKey];if(oldGroupKey===newGroupKey){group.Change(key,oldVal);return;}
group.Remove(key,oldVal);}
if(newGroupKey===undefined){return;}
group=_groups[newGroupKey];if(group===undefined){group=$L.Group(accessor,_groupSettings);_groups[newGroupKey]=group;}
group.Add(key);_keymap[key]=newGroupKey;}
accessor.on.insert(inFn);accessor.on.remove(remFn);accessor.on.change(changeFn);$L.set.meth(self,function ForceRescan(){accessor.each(function(val,key){var oldGroupKey,group;key=key.toString();var newGroupKey=_groupByFn(key,accessor);if($L.Map.has(_keymap,key)){oldGroupKey=_keymap[key];if(oldGroupKey===newGroupKey){return;}
group=_groups[oldGroupKey];group.Remove(key);}
if(newGroupKey===undefined){return;}
group=_groups[newGroupKey];if(group===undefined){group=$L.Group(accessor,_groupSettings);_groups[newGroupKey]=group;}
group.Add(key);_keymap[key]=newGroupKey;});});accessor.each(function(val,key){key=key.toString();var groupKey=_groupByFn(key,accessor);if(groupKey===undefined){return;}
var group=_groups[groupKey];if(group===undefined){group=$L.Group(accessor,_groupSettings);_groups[groupKey]=group;}
group.Add(key);_keymap[key]=groupKey;});return self;});});Lapiz.Module("Index",["Collections"],function($L){$L.set($L,"Index",function(cls,primaryFunc,domain){if(primaryFunc===undefined){primaryFunc=function(obj){return obj[$L.Index.defaultPrimary];};}else if($L.typeCheck.str(primaryFunc)){primaryFunc=function(field){return function(obj){return obj[field];};}(primaryFunc);}else if(!$L.typeCheck.func(primaryFunc)){Lapiz.Err.toss("Expected a function or string");}
if(domain===undefined){domain=cls;}else{cls[domain]=$L.Map();domain=cls[domain];}
var _primary=$L.Dictionary();domain.each=_primary.each;domain.has=_primary.has;domain.Filter=_primary.Filter;domain.Sort=_primary.Sort;domain.GroupBy=_primary.GroupBy;domain.remove=_primary.remove;Object.defineProperty(domain,"keys",{get:function(){return _primary.keys;}});Object.defineProperty(domain,"all",{get:function(){return _primary.Accessor;}});function _upsert(obj){_primary(primaryFunc(obj),obj);}
...
return _primary(idFuncOrAttr);};return cls;});$L.set.meth($L.Index,function Cls(constructor,primaryFunc,domain){return Lapiz.Index(Lapiz.Cls(constructor),primaryFunc,domain);});$L.Index.defaultPrimary="id";});Lapiz.Module("Collections",function($L){function Map(){return Object.create(null);}

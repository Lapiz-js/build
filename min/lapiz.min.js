...
$L.set.meth(self,function kill(){accessor.on.insert.deregister(inFn);accessor.on.remove.deregister(remFn);accessor.on.change.deregister(changeFn);if($L.typeCheck.nested(filterFn,"on","change","deregister","func")){filterFn.on.change.deregister(self.ForceRescan);}});Object.freeze(self);return self;});});Lapiz.Module("Group",function($L){$L.set($L,"Group",function(accessor){var _index=[];var self=function(key){if(_index.indexOf(key.toString())>-1){return accessor(key);}};$L.set(self,"_cls",$L.Group);$L.set(self,"Accessor",self);$L.set.meth(self,function Sort(funcOrField){return $L.Sort(self,funcOrField);});$L.set.meth(self,function Filter(filterOrField,val){return $L.Filter(self,filterOrField,val);});$L.set.meth(self,function has(key){return _index.indexOf(key.toString())>-1;});$L.set.getter(self,function keys(){return _index.slice(0);});$L.set.getter(self,function length(){return _index.length;});$L.set.meth(self,function each(fn){var i;var l=_index.length;for(i=0;i<l;i+=1){key=_index[i];if(fn(accessor(key),key)){return key;}}});$L.set(self,"on",$L.Map());var _insertEvent=$L.Event.linkProperty(self.on,"insert");var _changeEvent=$L.Event.linkProperty(self.on,"change");var _removeEvent=$L.Event.linkProperty(self.on,"remove");Object.freeze(self.on);function remFn(key,accessor,oldVal){key=key.toString();var i=_index.indexOf(key);if(i>-1){_index.splice(i,1);_removeEvent.fire(key,self,oldVal);}}
function changeFn(key,accessor,oldVal){key=key.toString();if(_index.indexOf(key)>-1){_changeEvent.fire(key,self,oldVal);}}
accessor.on.remove(remFn);accessor.on.change(changeFn);$L.set.meth(self,function kill(){accessor.on.remove.deregister(remFn);accessor.on.change.deregister(changeFn);});$L.set.meth(self,function Add(key){key=key.toString();if(accessor.has(key)){_index.push(key);_insertEvent.fire(key,self);}});$L.set.meth(self,function Remove(key){key=key.toString();var i=_index.indexOf(key);if(i>-1){_index.splice(i,1);_removeEvent.fire(key,self,accessor(key));}});Object.freeze(self);return self;});});Lapiz.Module("Index",["Collections"],function($L){$L.set($L,"Index",function(cls,primaryFunc,domain){if(primaryFunc===undefined){primaryFunc=function(obj){return obj[$L.Index.defaultPrimary];};}else if($L.typeCheck.str(primaryFunc)){primaryFunc=function(field){return function(obj){return obj[field];};}(primaryFunc);}else if(!$L.typeCheck.func(primaryFunc)){Lapiz.Err.toss("Expected a function or string");}

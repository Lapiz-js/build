...
node.removeAttribute("ifNot");if(attrVal){var parent=node.parentNode;parent.removeChild(node);UI.bindState.proceed=false;}});var repeaterProto=$L.Map();repeaterProto.registerAll=function(){var self=this;if(this.collection.on!==undefined){if($L.typeCheck.func(this.collection.on.insert)){this.deregisterInsFn=this.register(this.collection.on.insert,this.insFn);if((this.registrations&1)===0){UI.on.remove(this.parent,function(){self.deregisterInsFn();});this.registrations|=1;}}
if($L.typeCheck.func(this.collection.on.remove)){this.deregisterDelFn=this.register(this.collection.on.remove,this.delFn);if((this.registrations&2)===0){Lapiz.UI.on.remove(this.parent,function(){self.deregisterDelFn();});this.registrations|=2;}}
...
this.insFn(key,accessor);});repeaterProto.register=function(event,fn){event(fn);return function(){event.deregister(fn);};};$L.set.binder(repeaterProto,function append(val,key){var clone=this.templateNode.cloneNode(true);this.index[key]=clone;UI.bind(clone,val,this.templator);this.parent.insertBefore(clone,this.end);});repeaterProto.populate=function(){if(this.collection.each instanceof Function){this.collection.each(this.append);}else{$L.each(this.collection,this.append);}};UI.attribute("repeat",function(templateNode,ctx,collection){$L.assert(collection!==undefined,"Expected collection, got undefined: "+templateNode)
var repeater={"templateNode":templateNode,"collection":collection,"token":templateNode.attributes['repeat'].value,"templator":UI.bindState.templator,"index":$L.Map(),"parent":templateNode.parentNode,"registrations":0};repeater.end=templateNode.ownerDocument.createComment("end of "+repeater.token+" repeat")

...
$L.typeCheck(node,Node,"First argument to Lapiz.UI.getStyle must be node or valid selector string");return doc[docView].getComputedStyle(node,pseudoElt).getPropertyValue(property);})});Lapiz.Module("DefaultUIHelpers",["UI"],function($L){var UI=$L.UI;$L.UI.attribute("with",function(node,oldCtx,newCtx){$L.UI.bindState.ctx=newCtx;});UI.attribute("if",function(node,_,attrVal){if(typeof(attrVal)==="function"){attrVal=attrVal();}

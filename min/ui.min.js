...
if(node.nodeType===1||node.nodeType===11){for(cur=node.firstChild;cur!==null;cur=$L.UI.bindState.next){if(cur.tagName&&cur.tagName.toUpperCase()==="RENDER"){attrName=cur.attributes.getNamedItem('name').value;i=$L.UI.CloneView(attrName);cur.parentNode.insertBefore(i,cur.nextSibling);i=cur.nextSibling;cur.remove();cur=i;}
...
for(i=0;i<argsLen;i++){rend=_splitRenderString(arguments[i]);if(i===0){target=document.querySelector(rend.selector);append=rend.append;view=document.createDocumentFragment();view.appendChild(Lapiz.UI.CloneView(rend.view));Lapiz.UI.bind(view,ctx,Lapiz.Template.Std.templator);}else{if(rend.selector===""){rend.target=view;}else{rend.target=view.querySelector(rend.selector);}

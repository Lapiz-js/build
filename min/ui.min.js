...
return node;});ui.meth(function getStyle(node,property,doc,docView,pseudoElt){doc=doc||document;docView=docView||'defaultView';pseudoElt=pseudoElt||null;if($L.typeCheck.string(node)){node=doc.querySelector(node);}
...
return style;})
ui.meth(function Children(node){if($L.typeCheck.string(node)){node=document.querySelector(node);}
...
UI.bindState.firstPass&&node.addEventListener("click",fn);},"display":function(node,ctx,fn){if(typeof(fn)!=="function"){$L.Err.throw("Expected function");}
UI.bindState.firstPass&&fn(node,ctx);},"blur":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.throw("Expected function");}
UI.bindState.firstPass&&node.addEventListener("blur",fn);},"submit":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.throw("Expected function");}
UI.bindState.firstPass&&node.addEventListener("submit",fn);},"change":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.throw("Expected function");}

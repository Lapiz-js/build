...
return _props;}
var _views=$L.Map();var _viewAttribute='l-view';function _loadViews(){$L.each(document.querySelectorAll('['+_viewAttribute+']'),function(node){_views[node.attributes[_viewAttribute].value]=node;node.removeAttribute(_viewAttribute);node.remove();});}
...
target.appendChild(view);});ui.meth(function id(elId,doc){return(doc||document).getElementById(elId);});$L.UI.attribute("resolver",function(node,ctx,resolver){var _props=_getProperties(node);var templator=$L.Template.Templator($L.Template.Std.tokenizer,resolver);_props["templator"]=templator;$L.UI.bindState.templator=templator;});});Lapiz.Module("DefaultUIHelpers",["UI"],function($L){var UI=$L.UI;UI.attribute("if",function(node,_,attrVal){if(typeof(attrVal)==="function"){attrVal=attrVal();}
node.removeAttribute("if");if(!attrVal){var parent=node.parentNode;parent.removeChild(node);UI.bindState.proceed=false;}});UI.attribute("repeat",function(node,_,collection){var templator=UI.bindState.templator;if(collection===undefined){throw("Expected collection, got: "+collection);}
var insFn,delFn;var index=$L.Map();var parent=node.parentNode;var end=node.ownerDocument.createComment("end of repeat");parent.insertBefore(end,node);node.removeAttribute("repeat");var nodeTemplate=node.cloneNode(true);var fn=function(key,val){var clone=nodeTemplate.cloneNode(true);index[key]=clone;UI.bind(clone,val,templator);parent.insertBefore(clone,end);};if(collection.each instanceof Function){collection.each(fn);}else{$L.each(collection,fn);}
...
if($L.typeCheck.func(collection.on.change)&&delFn&&insFn){chgFn=function(key,obj,accessor){delFn(key,obj,accessor);insFn(key,accessor);}
collection.on.change(chgFn);UI.on.remove(parent,function(){collection.on.change.deregister(chgFn);});}}
node.parentNode.removeChild(node);UI.bindState.proceed=false;});var _liveNodes=new WeakMap();UI.attribute("live",function(node,context,altCtx){var ctx=altCtx||context;var fn;if(L.typeCheck.nested(ctx,"on","change","func")&&!_liveNodes.get(node)){_liveNodes.set(node,true);fn=function(){UI.bind(node);};ctx.on.change(fn);Lapiz.UI.on.remove(node,function(){ctx.on.change.deregister(fn);});}});UI.attribute({"click":function(node,_,fn){if(typeof(fn)!=="function"){throw new Error("Expected function");}
...
if(!fn(_getFormValues(form))&&evt&&evt.preventDefault){evt.preventDefault();}};});var _hash=$L.Map();UI.attribute("hash",function(node){var hash=node.getAttribute("hash");node.removeAttribute("hash");node.setAttribute("href","#"+hash);});UI.hash=function(hash,fn,ctx){var args=Array.prototype.slice.call(arguments);if(args.length===0){throw new Error("Hash requires at least one arg");}

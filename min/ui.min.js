...
return _views[name].cloneNode(true);});ui.meth(function Children(node){if($L.typeCheck.string(node)){node=document.querySelector(node);}
...
$L.typeCheck.string(mediatorName,"Mediator name must be a string");$L.assert(_mediators[mediatorName]===undefined,"Attempting to redefine "+mediatorName+" mediator");var properties=$L.Map();_mediators[mediatorName]={handler:fn,properties:properties};var registerFn=function(propName,prop){if(prop===undefined){if($L.typeCheck.func(propName)&&propName.name!==""){prop=propName;propName=prop.name;}else if($L.typeCheck.array(propName)){Lapiz.each(propName,function(fn){$L.typeCheck.func(fn,"On "+mediatorName+" found value in array that is not a function");$L.assert(fn.name!=='',"On "+mediatorName+" found unnamed function in array");properties[fn.name]=fn;});return;}else{Lapiz.each(propName,function(val,key){properties[key]=val;});return;}}
...
$L.Map.setterGetter($L.UI.bindState,"proceed",true,"bool");$L.Map.setterMethod($L.UI.bindState,function after(fn){_after.push(fn);});_props=_getProperties(node);if(_props['firstPass']===undefined){_props['firstPass']=false;$L.set($L.UI.bindState,'firstPass',true);}else{$L.set($L.UI.bindState,'firstPass',false);}
...
target.appendChild(view);});ui.meth(function id(elId,doc){return(doc||document).getElementById(elId);});ui.meth(function empty(node){if($L.typeCheck.string(node)){node=document.getElementById(node);}
...
return node;});$L.UI.attribute("resolver",function(node,ctx,resolver){var _props=_getProperties(node);var templator=$L.Template.Templator($L.UI.bindState.templator.tokenizer,resolver);_props["templator"]=templator;$L.UI.bindState.templator=templator;});ui.meth(function getStyle(node,property,doc,docView,pseudoElt){doc=doc||document;docView=docView||'defaultView';pseudoElt=pseudoElt||null;if($L.typeCheck.string(node)){node=doc.querySelector(node);}
...
return style;})});Lapiz.Module("DefaultUIHelpers",["UI"],function($L){var UI=$L.UI;$L.UI.attribute("with",function(node,oldCtx,newCtx){$L.UI.bindState.ctx=newCtx;});UI.attribute("if",function(node,_,attrVal){if(typeof(attrVal)==="function"){attrVal=attrVal();}
...
node.removeAttribute("ifNot");if(attrVal){var parent=node.parentNode;parent.removeChild(node);UI.bindState.proceed=false;}});UI.attribute("repeat",function(templateNode,_,collection){var templator=UI.bindState.templator;$L.assert(collection!==undefined,"Expected collection, got: "+collection)
...
$L.UI.bindState.firstPass&&node.addEventListener("click",fn);},"display":function(node,ctx,fn){if(typeof(fn)!=="function"){$L.Err.throw("Expected function");}
$L.UI.bindState.firstPass&&fn(node,ctx);},"blur":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.throw("Expected function");}
$L.UI.bindState.firstPass&&node.addEventListener("blur",fn);},"submit":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.throw("Expected function");}
$L.UI.bindState.firstPass&&node.addEventListener("submit",fn);},"change":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.throw("Expected function");}
$L.UI.bindState.firstPass&&node.addEventListener("change",fn);},"isChecked":function(node,ctx,bool){node.checked=!!bool;}});function _getForm(node){while(node.tagName!=="FORM"){node=node.parentNode;if(!("tagName"in node)){new Error("Node not in a form");}}
return node;}
function _getFormValues(form){var nameQuery=form.querySelectorAll("[name]");var i,n,nodeType;var data=$L.Map();for(i=nameQuery.length-1;i>=0;i-=1){n=nameQuery[i];nodeType=n.type;if(nodeType==="checkbox"||nodeType==="radio"){data[n.name]=n.checked;}else{data[n.name]=n.value;}}
return data;}
...
if(err){$L.Err.throw(err);}};});var _hash=$L.Map();UI.attribute("hash",function(node){var hash=node.getAttribute("hash");node.removeAttribute("hash");node.setAttribute("href","#"+hash);});UI.hash=function(hash,fn,ctx){var args=Array.prototype.slice.call(arguments);if(args.length===0){$L.Err.throw("Hash requires at least one arg");}
var hash=args.splice(0,1)[0];var fn=args[0];var ctx=args[1];if(args.length===0){Lapiz.each(hash,function(val,key){if(Array.isArray(val)&&val.length==2){UI.hash(key,val[0],val[1]);}else{UI.hash(key,val);}});}else if(typeof(args[0])==="string"){_hash[hash]=function(){UI.render.apply(this,args);};}else if(typeof(args[0])==="function"){_hash[hash]=fn;}};window.addEventListener("popstate",function(e){if(e.target!==undefined&&e.target.location!==undefined&&e.target.location.hash!==undefined){var args=e.target.location.hash.substr(1).split("/");var hash=args.shift();if(_hash[hash]!==undefined){_hash[hash].apply(this,args);}}});UI.on.loaded(function(){var args=document.location.hash.substr(1).split("/");var hash=args.shift();if(_hash[hash]!==undefined){_hash[hash].apply(this,args);}});UI.mediator("viewMethod",function viewMethod(node,ctx,methd){$L.typeCheck.func(methd,"Mediator viewMethod expects a function");return function innerViewMethod(){var args=Array.prototype.slice.call(arguments);args.splice(0,0,node,ctx);return methd.apply(this,args);};});var qRe={id:/\(?#(\w+)\)?/,cls:/\(?\.(\w+)\)?/g,attr:/\[(\w+)=([^\]]*)\]/g};UI.attribute("q",function(node,_,attrVal){var cls,attr,id;var clsVals=[node.className];if(clsVals[0]===''){clsVals=[];}

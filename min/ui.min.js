...
$L.each(_after,function(fn){fn();});_props['ctx']=$L.UI.bindState.ctx;_props['templator']=$L.UI.bindState.templator;$L.UI.bindState=$L.UI.bindState.parent;});var _mediatorRe=/^(\w+)\.(\w+)$/;function _getAttributeValue(str,ctx,node,templator){var mediatorPattern=_mediatorRe.exec(str);var mediator;if(mediatorPattern){mediator=_mediators[mediatorPattern[1]];if(mediator){return mediator.handler(node,ctx,mediator.properties[mediatorPattern[2]]);}}
...
return node;});$L.UI.attribute("resolver",function(node,ctx,resolver){var _props=_getProperties(node);var templator=$L.Template.Templator($L.UI.bindState.templator.tokenizer,resolver);_props["templator"]=templator;$L.UI.bindState.templator=templator;});});Lapiz.Module("DefaultUIHelpers",["UI"],function($L){var UI=$L.UI;$L.UI.attribute("with",function(node,oldCtx,newCtx){$L.UI.bindState.ctx=newCtx;});UI.attribute("if",function(node,_,attrVal){if(typeof(attrVal)==="function"){attrVal=attrVal();}

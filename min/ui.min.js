...
templateNode.remove();UI.bindState.proceed=false;});var _liveNodes=new WeakMap();UI.attribute("live",function(node,context,altCtx){var ctx=altCtx||context;var fn;if($L.typeCheck.nested(ctx,"on","change","func")&&!_liveNodes.get(node)){_liveNodes.set(node,true);fn=function(){UI.bind(node);};ctx.on.change(fn);Lapiz.UI.on.remove(node,function(){ctx.on.change.deregister(fn);});}});UI.attribute({"click":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.toss("Expected function");}
UI.bindState.firstPass&&node.addEventListener("click",fn);},"display":function(node,ctx,fn){if(typeof(fn)!=="function"){$L.Err.toss("Expected function");}
UI.bindState.firstPass&&fn(node,ctx);},"blur":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.toss("Expected function");}
UI.bindState.firstPass&&node.addEventListener("blur",fn);},"submit":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.toss("Expected function");}
UI.bindState.firstPass&&node.addEventListener("submit",fn);},"change":function(node,_,fn){if(typeof(fn)!=="function"){$L.Err.toss("Expected function");}
